...
require_once $libraryDir . '/Bing/Api/Exception/ApiConsumeException.php';
...
class ReverseIpLookup {
	...
	public function __construct($endpoint = NULL, $accountKey = NULL) {
		...
		if (!$this->isProcessed) {
			...
			for ($i = 0; $i < 2500; $i++) {
				...
				if (!is_string($response) && is_bool($response) && !$response) {
					throw new Exception\ApiConsumeException('Unable to access Bing API', 1373759960);
				}
